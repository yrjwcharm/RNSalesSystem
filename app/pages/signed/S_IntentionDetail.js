...
export default class S_IntentionDetail extends PureComponent {
    // 默认属性
    // 构造
    constructor(props) {
        super(props);
    ...
    _jumpToFollowInfoRouter=(id)=>{
        this.props.navigation.navigate('S_FollowInfo',{intentionId:id})
    }
    //跳转任务信息界面
    _jumpToTaskInfoRouter=(id)=>{
        this.props.navigation.navigate('S_TaskInfo',{intentionId:id})
    }
    //跳转联系人信息界面
    _jumpToContactInfoRouter=(id)=>{
        this.props.navigation.navigate('S_ContactInfo',{intentionId:id})
    }
    //跳转销售机会界面
    _jumpToSaleChanceRouter=(id)=>{
        this.props.navigation.navigate('S_SaleChance',{intentionId:id})
    }
    //跳转已签约合同界面
    _jumpToAlreadySignContractRouter=(id)=>{
        this.props.navigation.navigate('S_SignedContract',{intentionId:id})
    }
    //查询意向详情根据id
        ...
        fetch(url4,{method:'POST'}).then(res=>res.json()).then(responseText=>{
            this.setState({visible:false},()=>{
                if(responseText.success){
                    ...
                    // taskTypeName: "联系客户"
                    if(obj.length!==0){
                        const {planCompleteTime,planTimeStr,taskTypeName,taskDetail}=obj[0];
    ...
    // 渲染
    render() {
        const {planTimeStr,noLinkTime,customerLabelTypeId, taskList,contractList,
                ...
                <ScrollView keyboardShouldPersistTaps={'always'} showsVerticalScrollIndicator={false}>
                    <View style={{paddingHorizontal:moderateScale(13),marginTop:moderateScale(12)}}>
                        ...
                        <View style={{marginTop:moderateScale(11)}}>
                            ...
                            <Cell  onPress={()=>this._callPhone(telephone)} title={'手机'} value={StringUtils.isEmpty(telephone)?'暂无':telephone}/>
