...
export default class DelayTask extends PureComponent {
    // 默认属性
    // 构造
    constructor(props) {
        super(props);
    ...
    _save=()=>{
        const {userInfo:{userId},changeResult,delayTime}=this.state;
        const {params}=this.props.navigation.state;
        if(StringUtils.isEmpty(delayTime)){
            Toast.info('请选择延迟时间');
            return;
        }
        let url=Config.requestUrl+Config.taskDetailPageInterface.dealTask+`?userId=${userId}&taskId=${params.taskId}&operationType=4&changeResult=${changeResult}&delayToTime=${delayTime}`;
        console.log(333,url);
        fetch(url,{method:'POST'}).then(res=>res.json()).then(responseText=>{
    ...
    // 渲染
    render() {
        const {recipientPersonName,delayTime,executeTypeName,taskStatus,taskTypeName,planTimeStr,taskDetail}=this.state;
        return (
            <SafeAreaView style={{flex:1,backgroundColor:'#f2f2f2'}}>
                <Title title={'延期任务'} back onPressBack={() => this.props.navigation.goBack()}/>
                ...
                <Menu onPress={this._showDelayTimePicker} title={'推迟到'} detail={delayTime} style={{marginTop:px2dp(26)}}/>
