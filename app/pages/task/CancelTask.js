...
export default class CancelTask extends PureComponent {
    // 默认属性
    // 构造
    constructor(props) {
        super(props);
    ...
    _submit=()=>{
        ...
        const {params}=this.props.navigation.state;
        let url=Config.requestUrl+Config.taskDetailPageInterface.dealTask+`?userId=${userId}&taskId=${params.taskId}&operationType=3&changeResult=${changeResult}`;
        console.log(333,url);
        fetch(url,{method:'POST'}).then(res=>res.json()).then(responseText=>{
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{flex:1,backgroundColor:'#f2f2f2'}}>
                <Title title={'取消任务'} back onPressBack={() => this.props.navigation.goBack()}/>
                <View style={{marginTop: moderateScale(15), paddingHorizontal: moderateScale(13),}}>
                    <View style={{
                        ...
                        <DefaultInput title={'任务目的：'} value={StringUtils.isEmpty(taskTypeName)?'暂无':taskTypeName} noBorder/>
                ...
                <View style={{marginTop:px2dp(26),marginHorizontal: px2dp(26),backgroundColor:'#fff',borderRadius:px2dp(10)}}>
                    <Textarea rowSpan={8} onChangeText={(changeResult)=>{
                        this.setState({changeResult});
                    }} placeholder="任务处理结果" style={{paddingTop:px2dp(15),paddingLeft:px2dp(30),color:'#333',fontSize:px2dp(28)}} />
                </View>
